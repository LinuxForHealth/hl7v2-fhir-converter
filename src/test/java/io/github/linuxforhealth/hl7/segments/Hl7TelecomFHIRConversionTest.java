...
public class Hl7TelecomFHIRConversionTest {
  ...
  public void patient_telcom_test() {
    ...
    + "PID|1||12345678^^^^MR|ALTID|Moose^Mickey^J^III^^^||20060504|M|||||^PRN^PH^^22^555^1111313^^^^^^^^^^^2~^PRN^CP^^22^555^2221313^^^^^^^^^^^1|^PRN^PH^^^555^1111414^x889||||||||||||||||\n"
    ;
    ...
    assertThat(contact.getUse()).hasToString("HOME");
    ...
    assertThat(contact.getSystem()).hasToString("PHONE");

    // Second home contact
    contact = contacts.get(1); 
    assertThat(contact.getUse()).hasToString("MOBILE");
    ...
    assertThat(contact.getUse()).hasToString("WORK");
    assertThat(contact.getValue()).hasToString("(555) 111 1414 x889");
