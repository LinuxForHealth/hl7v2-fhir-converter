...
public class ProcedureUtils {
  ...
  public static Procedure createProcedureFromHl7Segment(String inputSegment){
    ...
    List<Resource> procedure =
        e.stream().filter(v -> ResourceType.Procedure == v.getResource().getResourceType())
            .map(BundleEntryComponent::getResource).collect(Collectors.toList());
    return getProcedureFromResource(procedure.get(0));
  }  

  private static Procedure getProcedureFromResource(Resource resource) {
