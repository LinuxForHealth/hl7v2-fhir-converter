...
public class FHIRExtensionsTest {
  ...
  @Test
  public void test_that_patient_with_no_extension_data_creates_no_extensions() {
    String patientWithNoExtensionData = "MSH|^~\\&|MIICEHRApplication|MIIC|MIIC|MIIC|201705130822||VXU^V04^VXU_V04|test1100|P|2.5.1|||AL|AL|||||Z22^CDCPHINVS|^^^^^MIIC^SR^^^MIIC|MIIC\n"
        // Test for no mother's maiden name, no religion, no race
        + "PID|1||12345678^^^^MR|ALTID|Mouse^Mickey^J^III^^^|^Mickette|20060504080400|M|Alias^Alias||12345 testing ave^^Minneapolis^MN^55407^^^^MN053|USAA|^PRN^^^PH^555^5555555|^PRN^^^PH^555^666666|english|married|ZZZ^No stated religion^|1234567_account|111-22-3333|DL00003333||2186-5^not Hispanic or Latino^CDCREC|Orlando Disney Hospital|Y|2|USA||||\n";

    Patient patient = PatientUtils.createPatientFromHl7Segment(patientWithNoExtensionData);
  ...
  public void test_that_race_extension_is_added_for_single_rep() {
    ...
    assertThat(coding.getSystem()).hasToString("urn:oid:2.16.840.1.113883.6.238");
  ...
  public void test_that_race_extension_is_added_for_multiple_rep() {
    ...
    assertThat(asianCoding.getSystem()).hasToString("urn:oid:2.16.840.1.113883.6.238");
    assertThat(whiteCoding.getDisplay()).hasToString("White");
    assertThat(whiteCoding.getCode()).hasToString("2106-3");
    assertThat(whiteCoding.getSystem()).hasToString("urn:oid:2.16.840.1.113883.6.238");

    subExts = ext.getExtensionsByUrl("text");
    assertThat(subExts).isNotEmpty().hasSize(1);
    assertThat(subExts.get(0).getValue()).hasToString("Mixed");
