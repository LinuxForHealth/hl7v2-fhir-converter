...
public class ImmunizationTest {
    ...
    public void testImmunization() throws IOException {
        ...
        assertThat(resource.getVaccineCode().getCoding().get(0).getSystem())
                .isEqualTo("http://hl7.org/fhir/sid/cvx"); // this is RXA.5 Should if  system is incorrect it  returns urn:extId:incorrectSystem....is this the  behavior  we want?
        ...
        assertThat(resource.getPerformer().get(0).getFunction().getCodingFirstRep().getCode()).isEqualTo("AP"); //RXA.10
        assertThat(resource.getPerformer().get(0).getFunction().getText())
                .isEqualTo("Administering Provider");// back  up value to ORC.12
