...
-  Set up your environment for Java development.  We have contributors using VS Code, Eclipse, and IntelliJ.
-  Run command `gradle clean build`.  This will confirm your libraries are set up correctly and your development environment is working.  Be prepared that testing step has 500+ tests and may take 10 minutes or more to complete.  
-  Select a unit test and run it.  This will confirm your unit test set up is working.
-  Set `localDevEnv = true` in `gradle.properties`.  This will help you see your changes without doing a full build.
-  View the json created from the conversion in the test.  The easiest way to do this is put a breakpoint immediately after the command `.convert`, which returns json. For most tests this is in a common method: `createFHIRBundleFromHL7MessageReturnEntryList` in `ResourceUtils.java`.
-  Use the debugger to step through the asserts of the test.  View the associated data structures.
-  Make a small change to one of the tests.  Change the input HL7 and see how the output works.  Repeat.
